...
def export_onnx(model_path, output_dir, imgsz, opset, simplify):
    ...
    unsupported_ops = set()
    # K1/RK3588 不友好的算子
    npu_unfriendly = {'Gather', 'GatherElements', 'ScatterND',
    ...
    if simplify:
        ...
        try:
            ...
            if check:
                ...
                print(f"     节点数: {orig_nodes} → {simp_nodes} "
                      f"(减少 {orig_nodes - simp_nodes} 个)")
            else:
                print(f"  ⚠️  onnxsim 简化验证失败，使用原始版本")
