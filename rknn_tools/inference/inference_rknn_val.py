...
parser.add_argument("--conf", default=OBJ_THRESH, type=float, help="置信度阈值（覆盖默认 OBJ_THRESH）")
...
def print_detection_eval(boxes, scores, classes, img_shape, ratio, dw, dh):
    ...
    labels = ["0.25-0.4", "0.4-0.5", "0.5-0.6", "0.6-0.7", "0.7-0.8", "0.8-0.9", "0.9-1.0"]
    print(f"\n  {'区间':<12} {'数量':>6}  {'占比':>6}")
    print(f"  {'-' * 30}")
...
def print_timing_eval(t_preprocess, t_infer, t_post, t_draw, times_infer=None, times_post=None, times_pipeline=None):
    """打印时间评估信息."""
    W = "=" * 54
    ...
    if times_infer is not None:
        ...
        print(f"  {'-' * 68}")
        for name, arr in [("NPU推理(ms)", times_infer), ("后处理(ms)", times_post), ("流水线(ms)", times_pipeline)]:
            p90 = np.percentile(arr, 90)
            p99 = np.percentile(arr, 99)
            print(
                f"  {name:<14} {arr.min():>7.2f} {arr.max():>7.2f} {arr.mean():>7.2f}"
...
if __name__ == "__main__":
    ...
    if args.bench > 0:
        ...
        for _ in range(args.bench):
            ...
            ti.append((t1 - t0) * 1000)
            tp.append((t2 - t1) * 1000)
            tpl.append((t2 - t0) * 1000)
        times_infer = np.array(ti)
        times_post = np.array(tp)
        times_pipeline = np.array(tpl)
        print("done")

    print_timing_eval(t_preprocess, t_infer, t_post, t_draw, times_infer, times_post, times_pipeline)
