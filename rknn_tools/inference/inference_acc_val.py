...
class StageTimer:
    ...
    def __init__(self, history_len: int = 200):
        self._t0: dict = {}
        self.history: dict = {s: collections.deque(maxlen=history_len) for s in [*self.STAGE_ORDER, "total"]}
...
class YOLOv12_ONNX_Inference:
    def __init__(
        ...
        """参数说明. -------- num_threads : ORT intra_op 线程数。SpaceMIT K1 (8核) 推荐先试 2 或 4。 use_io_binding : True = 启用 IO
        Binding，减少每帧输入/输出的内存拷贝开销。.
        """
        ...
        self._input_buf = np.empty((1, 3, self.input_height, self.input_width), dtype=np.float32)
    ...
    def preprocess(self, image: np.ndarray):
        ...
        # np.copyto 避免额外内存分配
        np.copyto(self._input_buf[0], (canvas[:, :, ::-1].transpose(2, 0, 1) / 255.0).astype(np.float32))
    ...
    def _run_with_io_binding(self, input_tensor: np.ndarray) -> np.ndarray:
        """IO Binding 让 ORT 直接操作我们提供的内存指针， 避免 session.run() 内部的数据拷贝开销。 对 SpaceMIT AI 扩展指令访问连续内存更友好。.
        """
    ...
    def postprocess(self, output, original_shape, scale, pad_w, pad_h):
        ...
        indices = cv2.dnn.NMSBoxes(cv_boxes_wh, score_f.tolist(), self.conf_thres, self.iou_thres)
        if len(indices) == 0:
            return [], [], []
    ...
    def benchmark_threads(
        ...
        image_path: str,
        input_size: tuple = (320, 320),
        thread_list: list | None = None,
        runs_per_cfg: int = 30,
    ):
        """对不同 num_threads 配置各跑 runs_per_cfg 帧，打印对比表， 找出推理延迟最低的线程数。.
        ...
        for n in thread_list:
            det = YOLOv12_ONNX_Inference(
                onnx_path, input_size=input_size, num_threads=n, warmup_runs=3, benchmark_history=runs_per_cfg
            )
            # 纯推理热身后压测
